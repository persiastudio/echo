...
function tinyecho_playback_start(filepath)
{
    ...
    var _size   = buffer_get_size(_raw) - 44;
    var _buffer = buffer_create(_size, buffer_fixed, 1);
    buffer_copy(_raw, 44, _size, _buffer, 0);
    buffer_delete(_raw);
	
	show_debug_message("[DEBUG] _size: " + string(_size));
	show_debug_message("[DEBUG] buffer: " + string(_buffer));
	show_debug_message("[DEBUG] buffer_size: " + string(buffer_get_size(_buffer)));
